...
that each of the two parts weighs even number of kilos, at the same
time it is not obligatory that the parts are equal. The boys are extremely
tired and want to start their meal as soon as possible, that's why you should
...
## Read W ==> number of Kilos
W = int(raw_input())

checked = 0

## check if the W can be sumbed using 2-even numbers
for even1 in range(1, W):
    if even1%2 == 0:
        for even2 in range(even1, W):
            if ( even2%2 == 0 and
                 even1+even2 == W):
                checked = 1
                break
