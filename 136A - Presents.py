...
else as what can be more pleasant than giving somebody gifts.
...
friends exchanging gifts while he does not participate in the process.
He numbered all his friends with integers from 1 to n. Petya remembered
that a friend number i gave a gift to a friend number pi. He also remembered
that each of his friends received exactly one gift.
...
the quantity of friends Petya invited to the party.
...
It is possible that some friends do not share Petya's ideas of giving
...
## Read nu,ber of friends
N = raw_input()
N = int(N)

## iantilize friends list
friends_li = [0] * N
## read frinds numbers from user
one_element = raw_input().split()
one_element = map(int, one_element)

## the i-th number should equal the number of
##      the friend who gave a gift to friend number i
for i in range(0, N):
    one_tmp = one_element[i]
    friends_li[one_tmp - 1] = i + 1

## print list seperated by space
print ' '.join(map(str, friends_li))
