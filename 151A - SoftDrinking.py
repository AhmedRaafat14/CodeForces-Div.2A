...
Also they bought c limes and cut each of them into d slices.
...
The limes are enough for 10 * 8 = 80 toasts and
the salt is enough for 100 / 1 = 100 toasts.
However, there are 3 friends in the group,
...
N, K, L, C = int(data[0]), int(data[1]), int(data[2]), int(data[3])
D, P, NL, NP = int(data[4]), int(data[5]), int(data[6]), int(data[7])
...
print(min(drink_toasts, limes_toasts, salt_toasts) // N)
